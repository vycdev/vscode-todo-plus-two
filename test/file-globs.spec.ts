...
describe('Workspace file excludes', () => {
  it('normalizes cached and open document paths with filesystem casing rules', () => {
    expect(getFilePathKey('C:\\Workspace\\app.ts', 'win32')).to.equal(
      getFilePathKey('c:/workspace/app.ts', 'win32')
    );
    expect(getFilePathKey('/Workspace/app.ts', 'linux')).not.to.equal(
      getFilePathKey('/workspace/app.ts', 'linux')
    );
  });
  ...
  it('matches workspace roots only at path boundaries', () => {
    ...
    expect(rootsRe!.exec('/workspace/packages/app/TODO')![1]).to.equal('/workspace/packages/app');
  ...
  it('matches workspace root regular expression casing according to the filesystem', () => {
    expect(getRootPathsRegExp(['C:/Workspace'], 'win32')!.test('c:/workspace/TODO')).to.equal(true);
    expect(getRootPathsRegExp(['/Users/Alice'], 'darwin')!.test('/users/alice/TODO')).to.equal(
      true
    );
    expect(getRootPathsRegExp(['/Workspace'], 'linux')!.test('/workspace/TODO')).to.equal(false);
  });
  ...
  it('applies workspace-relative excludes alongside todo globs', () => {
    ...
    expect(isFileIncluded(path.join(rootPath, 'src', 'TODO'), rootPath, include, exclude)).to.equal(
      true
    );
    expect(
  ...
  it('applies conditional excludes when the configured sibling exists', () => {
    ...
    try {
      ...
      expect(isFileIncluded(javascriptPath, rootPath, ['**/*.js'], [], exclude)).to.equal(true);

      fs.writeFileSync(typescriptPath, '');
      expect(isFileIncluded(javascriptPath, rootPath, ['**/*.js'], [], exclude)).to.equal(false);
  ...
  it('uses filesystem case sensitivity for glob matching', () => {
    ...
    expect(micromatch(['README.TXT'], ['**/*.txt'], getGlobMatchOptions('win32'))).to.deep.equal([
      'README.TXT',
    ]);
    expect(micromatch(['README.TXT'], ['**/*.txt'], getGlobMatchOptions('linux'))).to.deep.equal(
      []
    );
  });
});
