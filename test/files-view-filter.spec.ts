...
describe('Files view filter', () => {
  it('matches task text case-insensitively and treats the filter literally', () => {
    expect(matchesFilesViewFilter('URGENT', '/workspace/TODO', ['  ☐ urgent task'])).to.equal(true);
    expect(matchesFilesViewFilter('[api]', '/workspace/TODO', ['  ☐ fix [api]'])).to.equal(true);
  ...
  it('shows every branch when the file path matches', () => {
    expect(
      matchesFilesViewFilter('client/todo', '/workspace/client/TODO', ['Unrelated:', '  ☐ task'])
    ).to.equal(true);
  });
  ...
  it('treats finished-tag examples inside inline code as unfinished', () => {
    ...
    expect(hasUnfinishedTodo(['  ☐ explain `@done`'], todoPattern, finishedTodoPattern)).to.equal(
      true
    );
    expect(hasUnfinishedTodo(['  ☐ real status @done'], todoPattern, finishedTodoPattern)).to.equal(
      false
    );
  });
});
