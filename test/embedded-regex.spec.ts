...
describe('Embedded todo regex defaults', () => {
  ...
  ['ag', 'rg'].forEach((provider) => {
    it(`includes Liquid comments in the ${provider} candidate regex`, () => {
      ...
      expect(regex.test('{% comment %} TODO: update the product copy {% endcomment %}')).to.equal(
        true
      );
    });
...
describe('Embedded todo line parsing', () => {
  it('handles empty global matches and optional or absent marker captures', () => {
    expect(parseEmbeddedMatches('TODO hello', /(?:)/g)).to.deep.equal([]);
    expect(parseEmbeddedMatches('TODO hello', /TODO/g)).to.deep.equal([]);
    expect(parseEmbeddedMatches('hello', /(TODO)?(hello)/g)).to.deep.equal([]);
    expect(parseEmbeddedMatches('😀 TODO', /(?:|(TODO))/gu)).to.deep.equal([]);
    expect(parseEmbeddedMatches(' TODO', /(TODO)|(?:)/g)).to.have.length(1);
  });

  it('does not reuse mutable lastIndex from a custom sticky expression', () => {
    const regex = /(TODO)(.*)/y;
    regex.lastIndex = 5;
    expect(parseEmbeddedMatches('TODO hello', regex)).to.have.length(1);
    expect(regex.lastIndex).to.equal(5);
  });
