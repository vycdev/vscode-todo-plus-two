...
describe('Embedded todo context', () => {
  ...
  it('returns the source line immediately following an embedded todo', () => {
    ...
    expect(getFollowingContext(lines, 0, isEmbeddedTodo)).to.equal('const result = buildValue();');
  });
  ...
  it('returns no context at the end of a file', () => {
    expect(getFollowingContext(['// TODO: simplify this'], 0, isEmbeddedTodo)).to.equal(undefined);
  });
});
