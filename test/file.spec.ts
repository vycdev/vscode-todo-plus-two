  ...
  it('centers the selected range when opening a text document', async () => {
    const document = {},
      ...
      vscode = {
        ...
        window: {
          activeTextEditor: { selection: undefined },
    ...
    const openedEditor = await File.open('/workspace/tasks.todo', true, 12, 3, 8);

    expect(openedEditor).to.equal(editor);
    expect(vscode.window.activeTextEditor.selection).to.equal(undefined);
