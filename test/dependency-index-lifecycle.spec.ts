...
describe('Dependency index lifecycle', () => {
  ...
  it('does not publish an in-flight refresh after disposal and can initialize again', async () => {
    const { DependencyIndex, getClearCount } = loadDependencyIndex(),
      subscriptions = [];
    let resolveIndex;

    DependencyIndex.get = () => new Promise((resolve) => (resolveIndex = resolve));
    DependencyIndex.initialize({ subscriptions });
    subscriptions.forEach((subscription) => subscription.dispose());
    resolveIndex({ targets: {}, dependencies: {} });
    await Promise.resolve();
    await Promise.resolve();

    expect(getClearCount()).to.equal(0);
    expect(DependencyIndex.diagnostics).to.equal(undefined);

    const nextSubscriptions = [];
    DependencyIndex.initialize({ subscriptions: nextSubscriptions });
    expect(nextSubscriptions).not.to.be.empty;
    nextSubscriptions.forEach((subscription) => subscription.dispose());
    resolveIndex({ targets: {}, dependencies: {} });
  });
});
