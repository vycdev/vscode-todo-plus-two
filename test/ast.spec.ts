...
const loadAst = (visibleTextEditors = []) => {
      ...
      return {
        default: {
          getIndentation: (editor) =>
            editor.options ? ' '.repeat(editor.options.tabSize) : undefined,
        },
      };
    }
...
describe('AST indentation detection', () => {
  it('honors editor indentation changes without requiring a document edit', () => {
    const document = {
        fileName: '/workspace/TODO',
        getText: () => 'Project:\n    ☐ Task',
        version: 1,
      },
      editor = { document, options: { tabSize: 4 } },
      AST = loadAst([editor]);

    expect(AST.getLevel(document, '    ☐ Task')).to.equal(1);
    editor.options.tabSize = 2;
    expect(AST.getLevel(document, '    ☐ Task')).to.equal(2);
  });
