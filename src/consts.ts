    ...
    get() {
        ...
        return {
            ...
            regexes: {
                impossible: /(?=a)b/gm,
                empty: /^\s*$/,
                // Recognize the three Todo+ symbols: box (☐), done (✔) and cancelled (✘)
                // Also accept Markdown checkbox forms, optionally preceded by a list marker like "- [ ]" or "- [x]".
                // The bracket form is captured as a whole (e.g. "- [ ]" or "[x]").
                todo: /^[^\S\n]*((?!--|––|——)(?:[☐✔✘]|(?:-\s+)\[[ xX]\])\s[^\n]*)/gm,
                todoSymbol: /^[^\S\n]*(?!--|––|——)((?:[☐✔✘]|(?:-\s+)\[[ xX]\]))\s/,
                todoBox:
                ...
                tagSpecial: new RegExp(
                    `(?:^|[^a-zA-Z0-9])@(${tagsNames.map((n) => _.escapeRegExp(n)).join('|')})(?:(?:\\([^)]*\\))|(?![a-zA-Z]))`,
                    'gm'
                ),
                tagSpecialNormal: new RegExp(
                    `(?:^|[^a-zA-Z0-9])(?:${tagsNames.map((n) => `(@${_.escapeRegExp(n)}(?:(?:\\([^)]*\\))|(?![a-zA-Z])))`).join('|')}|(@[^\\s*~(]+(?::\/\/[^\\s*~(:]+)?(?:(?:\\([^)]*\\))|(?![a-zA-Z]))))`,
                    'gm'
                ),
                tagNormal: new RegExp(
                    `(?:^|[^a-zA-Z0-9])@(?!${tagsNames.map((n) => _.escapeRegExp(n)).join('|')}|created|done|cancelled|started|lasted|wasted|est|\\d)[^\\s*~(:]+(?::\/\/[^\\s*~(:]+)?(?:\\([^)]*\\))?`
                ),
