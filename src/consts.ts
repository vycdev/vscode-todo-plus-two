...
import { createConfiguredRegex } from './utils/configured-regex';
...
const Consts = {
  get() {
    ...
    const regexes = {
      ...
      todoEmbedded: createConfiguredRegex(embeddedRegex, embeddedFlags, (message) =>
      ...
      tagCreated: new RegExp(`(?:^|[^a-zA-Z0-9])@created${capturedReservedTagArgumentOrBoundary}`),
      tagDue: /(?:^|[^a-zA-Z0-9])(@due\([^)]*\))/gim,
      tagStarted: new RegExp(`(?:^|[^a-zA-Z0-9])@started${capturedReservedTagArgumentOrBoundary}`),
