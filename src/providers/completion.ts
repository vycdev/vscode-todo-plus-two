...
class Completion implements vscode.CompletionItemProvider {
  ...
  async provideCompletionItems(textDocument: vscode.TextDocument, pos: vscode.Position) {
    ...
    const dependency = getTagArgumentPrefix(line.substring(0, pos.character), pos.character);

    if (dependency && dependency.name.toLowerCase() === '@depends') {
      return Completion.getDependencyIds(
        textDocument,
        pos,
        dependency.text.slice(dependency.text.indexOf('(') + 1)
      );
    }
    ...
    if (
      ...
      const timestampTags = Timestamps.aliases.map((text) => {
        ...
        item.detail = text === '@now' ? 'Insert current timestamp' : 'Insert @created timestamp';
        item.filterText = text;
  ...
  private static async getDependencyIds(
    ...
    const range = new vscode.Range(
      ...
      pos.line,
      pos.character + (textDocument.lineAt(pos.line).text[pos.character] === ')' ? 1 : 0)
    );
